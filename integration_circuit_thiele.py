...
Integration circuit modified from:
...
modified  by Alexander Thiele to represent 'attention source circuit' 
with reduced winner take all characteristics and reduced NMDA drive in small populations
...
def make_integration_circuit():
    ...
    N = int(2000  *    size_scale)                                # Total number of neurons
    ...
    gEE_AMPA = 0.12 * nS		                 # Weight of AMPA synapses between excitatory neurons
    gEE_NMDA = 0.145 * nS                        # Weight of NMDA synapses between excitatory neurons
    gEI_AMPA = 0.11 * nS                         # Weight of excitatory to inhibitory synapses (AMPA)
    gEI_NMDA = 0.12 * nS                         # Weight of excitatory to inhibitory synapses (NMDA)
    gIE_GABA = 1.295 * nS                          # Weight of inhibitory to excitatory synapses
    ...
    def update_nmda():
        ...
        E1_gen[0:10] = 0.6*gEE_NMDA / gLeakE * (w_p*sE1 + w_m*sE2 + w_m*sE3)
        E1_gen[10:20] = 0.75*gEE_NMDA / gLeakE * (w_p*sE1 + w_m*sE2 + w_m*sE3)
        E1_gen[20:30] = 0.9*gEE_NMDA / gLeakE * (w_p*sE1 + w_m*sE2 + w_m*sE3)
        E2_gen[:] = gEE_NMDA / gLeakE * (w_m*sE1 + w_p*sE2 + w_m*sE3)
        E2_gen[0:10] = 0.6*gEE_NMDA / gLeakE * (w_p*sE1 + w_m*sE2 + w_m*sE3)
        E2_gen[10:20] = 0.75*gEE_NMDA / gLeakE * (w_p*sE1 + w_m*sE2 + w_m*sE3)
        E2_gen[20:30] = 0.9*gEE_NMDA / gLeakE * (w_p*sE1 + w_m*sE2 + w_m*sE3)
        E3_gen[:] = gEE_NMDA / gLeakE * (sE1 + sE2 + sE3)
        I_gen[:] = gEI_NMDA / gLeakI * (sE1 + sE2 + sE3)    
    
    # Connections involving GABA synapses
    C_DI_DE = Connection(decisionI, decisionE, 'gi', sparseness=0.5,weight = gIE_GABA / gLeakE, delay = d)
    C_DI_DI = Connection(decisionI, decisionI, 'gi', sparseness=0.5,weight = gII_GABA / gLeakI, delay = d)
